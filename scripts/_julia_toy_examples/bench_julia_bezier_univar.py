# PYTHON_JULIACALL_HANDLE_SIGNALS=yes JULIA_NUM_THREADS=4 python3 scripts/_julia_toy_examples/bench_julia_bezier_univar.py --degree 20 --bezier-degree 3 --n-eval 1000
import time
import numpy as np
...
def main(degree=10, bezier_degree=4, n_eval=200, seed=0, compute_newton_iters=False):
    ...
    t0 = now()
    jl.seval('include("scripts/_julia_toy_examples/bezier_univar.jl")')
    t_include = now() - t0
