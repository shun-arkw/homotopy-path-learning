...
function randomize_middle!(H::BezierSparse2x2Deg3)
    m = H.m
    @inbounds for j in 2:m
        for eq in 1:2, term in 1:3
            H.ctrl[eq,term,j] = 0.2 * (randn() + randn()*im)
        end
        H.ctrl[1,1,j] += 1.0 + 0im
        H.ctrl[2,1,j] += 1.0 + 0im
    end
    compute_diffs!(H.diffs, H.ctrl, H.m, H.kmax)
    return nothing
end

function perturb_middle!(H; σ=0.02)
    m = H.m
    @inbounds for j in 2:m, eq in 1:2, term in 1:3
        H.ctrl[eq,term,j] += σ * (randn() + randn()*im)
    end
    compute_diffs!(H.diffs, H.ctrl, H.m, H.kmax)
end
...
function main(; m::Int=4, n_eval::Int=200, seed::Int=0)
    ...
    times = Vector{Float64}(undef, n_eval)
    steps = Vector{Int}(undef, n_eval)
    okcnt = 0

    for it in 1:n_eval
        # randomize_middle!(H)
        perturb_middle!(H; σ=0.1)
    ...
    mean_t = sum(times) / n_eval
    mean_s = sum(steps) / n_eval
...
function parse_args(args)
    ...
    return (m=m, n=n, seed=seed)
end

if abspath(PROGRAM_FILE) == @__FILE__
    a = parse_args(ARGS)
    main(m=a.m, n_eval=a.n, seed=a.seed)
end
