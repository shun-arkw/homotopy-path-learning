...
- Bézier degree を **d** とする（$d\geq 2$ の整数．例：2, 3, 4, ...）．
...
本章では，Bézier degree $d\geq 2$ の場合に，action をどのように定義し，1エピソード内で何回 solver を走らせるかを整理する．$d=2,3$ は典型的な例であるが，$d\geq 4$ も同様にサポートする．
...
#### 4.1.3 $d\geq 4$ の場合
- 中間制御点は $d-1$ 個（$P_1,\dots,P_{d-1}$）である．
- action は $\mathbf{a}=(z_1,\dots,z_{d-1})\in\mathbb{R}^{(d-1)m}$ を一括で出力する．実装では $d=2,3$ と同様にサポートされる．
...
- Bézier degree は $d\geq 2$ の整数とし，中間制御点を一括更新する．
