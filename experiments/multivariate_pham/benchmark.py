...
from homotopy_path_learning.evaluation import benchmark_checkpoint
from homotopy_path_learning.rl.train_utils import refresh_metadata_output_files


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Linear, RandomBezier, and LearnedBezier.")
    ...
    summary_csv = run_dir / "benchmark" / "benchmark_summary.csv"
    rows, summaries = benchmark_checkpoint(
        config=config,
        run_id=str(metadata["run_id"]),
        checkpoint_path=args.checkpoint,
        output_path=benchmark_csv,
    ...
    print(f"BENCHMARK_SUMMARY_ROWS={len(summaries)}")
